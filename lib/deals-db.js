...
const toDeal = (r) => r && ({
  ...
  lastTouchpoint: r.last_touchpoint,      // 'YYYY-MM-DD' — never a local Date
...
/* One template with self-neutralising parameters rather than a complete
   query per filter combination — there are four independent filters, which
   is a dozen near-identical blocks the other way. Nothing is concatenated;
   the ::text casts are what let Postgres infer each comparison. */
export async function listDeals({ stage = 'all', q = '', owner = 'all', escalated = false, year } = {}) {
  ...
  const rows = await sql`
    ...
    WHERE (${stage}::text = 'all' OR d.stage = ${stage})
    ...
    ORDER BY d.escalated DESC,
             array_position(ARRAY['won','terms','proposal','active','engaged']::text[], d.stage),
             d.company ASC
  `;
...
export async function getDeal(id) {
  const [row] = await sql`
    SELECT d.*, p.company AS parent_company
...
export async function dealCounts(year) {
  ...
  const [extra] = await sql`
    SELECT (SELECT count(*)::int FROM deals) AS total,
           (SELECT count(*)::int FROM deals WHERE escalated) AS escalated,
           COALESCE((SELECT sum(amount)::int FROM deal_years
                     WHERE guaranteed AND (${year ?? null}::int IS NULL OR year = ${year ?? null})), 0) AS guaranteed,
           COALESCE((SELECT sum(amount)::int FROM deal_years
                     WHERE NOT guaranteed AND (${year ?? null}::int IS NULL OR year = ${year ?? null})), 0) AS optioned
  `;
  return {
    byStage: Object.fromEntries(STAGES.map((s) => [s.key, byStage[s.key] || 0])),
    total: extra.total,
...
export async function updateDeal(actor, id, fields) {
  ...
  const [before] = await sql`SELECT * FROM deals WHERE id = ${dealId}`;
...
export async function chainFor(dealId) {
  const rows = await sql`
    SELECT * FROM deals WHERE parent_deal_id = ${Number(dealId) || 0} ORDER BY id
  `;
  return rows.map(toDeal);
}
