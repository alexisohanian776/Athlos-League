...
/* Who can be a sales lead. Two sources, because they are genuinely
   different: the names already on deals came off the sheet and mostly have
   no account here, while the staff roster has people who have not been given
   a deal yet. Suggesting only the first means a new lead can never be picked
   until someone types their name correctly once. */
export async function dealLeadNames() {
  const [onDeals, staff] = await Promise.all([
    sql`SELECT DISTINCT owner_name AS name FROM deals WHERE owner_name IS NOT NULL AND owner_name <> ''`,
    sql`SELECT first_name, last_name FROM users WHERE role <> 'fan' AND NOT disabled`,
  ]);
  const names = new Set(onDeals.map((r) => r.name));
