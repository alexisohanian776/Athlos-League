...
export async function dealCounts(year) {
  ...
  const [extra] = await sql`
    ...
    FROM (
      ...
      WHERE d.stage NOT IN (SELECT json_array_elements_text(${JSON.stringify(CLOSED_KEYS)}::json))
...
export async function setDealYear(actor, dealId, { year, amount, guaranteed, kind }) {
  const deal = Number(dealId) || 0;
