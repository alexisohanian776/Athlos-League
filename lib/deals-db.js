...
/* Companies on the list nobody has approached yet. Off the funnel for the
   same reason the closed stages are: 202 cold names would drown the deals
   actually being worked, and they are not money in play. */
export const PROSPECT_STAGE = { key: 'prospect', label: 'Prospect', hint: 'On the list, not yet approached.' };
...
export const ALL_STAGES = [PROSPECT_STAGE, ...STAGES, ...CLOSED_STAGES];
/* Everything the default list and the money totals leave out. */
export const ASIDE_STAGES = [PROSPECT_STAGE, ...CLOSED_STAGES];
export const ASIDE_KEYS = ASIDE_STAGES.map((s) => s.key);
...
export const isAsideStage = (key) => ASIDE_KEYS.includes(key);
...
const RANKED_JSON = ['prospect', ...[...CLOSED_KEYS].reverse()]
  ...
  const rows = await sql`
    SELECT * FROM (
      ...
      WHERE (
          ...
          (${stage}::text = 'all'
            AND d.stage NOT IN (SELECT json_array_elements_text(${JSON.stringify(ASIDE_KEYS)}::json)))
...
export async function dealCounts(year) {
  ...
  /* Money in play means live deals. A lost sponsor's number staying in the
     total is the whole reason the closed stages are a separate list. */
  const [extra] = await sql`
    SELECT (SELECT count(*)::int FROM deals
            WHERE stage NOT IN (SELECT json_array_elements_text(${JSON.stringify(ASIDE_KEYS)}::json))) AS total,
           ...
           (SELECT count(*)::int FROM deals WHERE stage = 'prospect') AS prospects,
           m.*
    FROM (
      SELECT COALESCE(sum(y.amount) FILTER (WHERE y.kind = 'cash'  AND y.guaranteed), 0)::int AS cash,
             COALESCE(sum(y.amount) FILTER (WHERE y.kind = 'vik'   AND y.guaranteed), 0)::int AS vik,
             COALESCE(sum(y.amount) FILTER (WHERE y.kind = 'media' AND y.guaranteed), 0)::int AS media,
             COALESCE(sum(y.amount) FILTER (WHERE y.guaranteed), 0)::int     AS guaranteed,
             COALESCE(sum(y.amount) FILTER (WHERE NOT y.guaranteed), 0)::int AS optioned
      FROM deal_years y JOIN deals d ON d.id = y.deal_id
      WHERE d.stage NOT IN (SELECT json_array_elements_text(${JSON.stringify(ASIDE_KEYS)}::json))
...
export async function setDealYear(actor, dealId, { year, amount, guaranteed, kind, note }) {
  ...
  await sql`
    ...
    VALUES (${deal}, ${yr}, ${Math.round(value)}, ${what}, ${Boolean(guaranteed)},
