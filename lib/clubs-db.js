...
export async function createClub(c) {
  ...
  const rows = await sql`
    INSERT INTO run_clubs
      (slug, name, type, city, code, region, hood, organizer, members, next_run, tone, map_x, map_y, founded, photo_url, about, website, instagram, sort_order)
    VALUES
      (${slug}, ${c.name}, ${c.type}, ${c.city}, ${c.code}, ${c.region}, ${c.hood},
       ${c.organizer}, ${c.members}, ${c.next}, ${c.tone}, ${c.mapX}, ${c.mapY}, ${c.founded}, ${c.photo},
       ${c.about}, ${c.website}, ${c.instagram},
...
export async function updateClub(id, c) {
  ...
  const rows = await sql`
    UPDATE run_clubs SET
      ...
      founded = ${c.founded}, photo_url = ${c.photo},
      about = ${c.about}, website = ${c.website}, instagram = ${c.instagram},
      updated_at = now()
    WHERE id = ${id}
    RETURNING *
  `;
...
/* Lookup by id, for a fan's club affiliation — users store club_id. */
export async function getClubById(id) {
  const rows = await sql`SELECT * FROM run_clubs WHERE id = ${Number(id)}`;
  return rows[0] ? toClub(rows[0]) : null;
}
