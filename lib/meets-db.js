...
/* Meet records are counted from the results rather than typed in: an
   athlete's mark carries 'MR' when it set one, so the number cannot drift
   away from the results table the way a hand-entered figure would. */
function meetRecordsFor(year) {
  return (MEET_RESULTS[year] || []).reduce(
    (n, ev) => n + ev.athletes.filter((a) => a.marks?.includes('MR')).length,
    0
  );
}
...
const toMeet = (r) => r && ({
  ...
  headline: r.headline || '',
  weather: r.weather || '',
  prizePurse: r.prize_purse || '',
  meetRecords: meetRecordsFor(r.year),
...
export async function updateMeet(id, m) {
  const rows = await sql`
    UPDATE meets SET
      ...
      venue = ${m.venue}, city = ${m.city}, country = ${m.country},
      attendance = ${m.attendance}, events = ${m.events},
      headline = ${m.headline}, weather = ${m.weather || null},
      prize_purse = ${m.prizePurse || null},
...
export async function createMeet(m) {
  const rows = await sql`
    INSERT INTO meets (slug, name, year, held_on, venue, city, country,
                       attendance, events, headline, weather, prize_purse,
                       photo_url, tone, published)
    VALUES (${m.slug}, ${m.name}, ${m.year}, ${m.heldOn || null}, ${m.venue},
            ${m.city}, ${m.country}, ${m.attendance}, ${m.events},
            ${m.headline}, ${m.weather || null}, ${m.prizePurse || null},
            ${m.photoUrl || null}, ${m.tone}, ${Boolean(m.published)})
...
/* The homepage archive: past, published meets with the fans actually
   verified for each. Formatting happens here so the client never turns an
   ISO date into a local one and disagrees with the server. */
export async function pastMeetsForArchive() {
  const rows = await sql`
    SELECT m.*, to_char(m.held_on, 'YYYY-MM-DD') AS held_on,
           to_char(m.held_on, 'FMMon FMDD, YYYY') AS date_label,
           (SELECT count(*)::int FROM attendances a WHERE a.meet_id = m.id AND a.status = 'approved') AS verified_count
    FROM meets m
    WHERE m.published AND m.held_on IS NOT NULL AND m.held_on < current_date
    ORDER BY m.held_on DESC`;

  const meets = rows.map((r) => ({ ...toMeet(r), dateLabel: r.date_label }));
  if (!meets.length) return meets;

  /* Up to six faces per meet, newest verified first. */
  const fans = await sql`
    SELECT a.meet_id, u.first_name, u.last_name, u.email, u.handle, u.avatar_url
    FROM attendances a JOIN users u ON u.id = a.user_id
    WHERE a.status = 'approved' AND a.meet_id = ANY(${meets.map((m) => m.id)})
    ORDER BY a.reviewed_at DESC NULLS LAST`;

  const byMeet = new Map();
  for (const f of fans) {
    const list = byMeet.get(f.meet_id) || [];
    if (list.length < 6) {
      list.push({
        name: [f.first_name, f.last_name].filter(Boolean).join(' ') || f.email.split('@')[0],
        handle: f.handle,
        avatarUrl: f.avatar_url,
      });
    }
    byMeet.set(f.meet_id, list);
  }
  return meets.map((m) => ({ ...m, fans: byMeet.get(m.id) || [] }));
}
