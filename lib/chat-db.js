...
/* How many messages this person has not seen. Their own messages never
   count — you have read what you just wrote. */
export async function unreadFor(userId) {
  const [row] = await sql`
    SELECT count(*)::int AS n
    FROM chat_messages m
    WHERE m.user_id <> ${userId}
      AND m.id > (SELECT chat_read_id FROM users WHERE id = ${userId})
  `;
  return row?.n || 0;
}

/* Marks everything up to `upTo` seen. Never moves backwards, so an old tab
   finishing a slow request cannot un-read newer messages. */
export async function markRead(userId, upTo) {
  const id = Number(upTo);
  if (!Number.isInteger(id) || id < 0) return;
  await sql`
    UPDATE users SET chat_read_id = GREATEST(chat_read_id, ${id})
    WHERE id = ${userId}
  `;
}
