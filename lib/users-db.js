...
export async function updateProfile(id, {
  firstName, lastName, email, city, bio, instagram, strava, clubId, favourites, handle,
}) {
  ...
  /* Handle is the public URL, so it gets the same checks as at sign-up. */
  let h;
  if (handle !== undefined && handle !== null && String(handle).trim() !== '') {
    const problem = handleProblem(handle);
    if (problem) return { error: 'handle', message: problem };
    h = String(handle).trim().toLowerCase();
    const taken = await sql`SELECT id FROM users WHERE lower(handle) = ${h} AND id <> ${id}`;
    if (taken.length) return { error: 'handle', message: 'That handle is taken.' };
  }

  /* Only slugs, capped at five, duplicates dropped, order kept. */
  const faves = Array.isArray(favourites)
    ? [...new Set(favourites.map((f) => String(f).trim()).filter(Boolean))].slice(0, 5)
    : null;

  const club = clubId === '' || clubId === null || clubId === undefined ? null : Number(clubId);

  const display = [first, last].filter(Boolean).join(' ') || null;
  await sql`
    UPDATE users SET first_name = ${first || null}, last_name = ${last || null},
      name = ${display}, email = ${mail},
      handle = COALESCE(${h ?? null}, handle),
      city = ${city === undefined ? null : String(city || '').trim().slice(0, 120) || null},
      bio = ${bio === undefined ? null : String(bio || '').trim().slice(0, 280) || null},
      instagram = ${instagram === undefined ? null : String(instagram || '').trim().slice(0, 80) || null},
      strava = ${strava === undefined ? null : String(strava || '').trim().slice(0, 200) || null},
      club_id = ${Number.isInteger(club) ? club : null},
      favourites = COALESCE(${faves ? JSON.stringify(faves) : null}::jsonb, favourites),
      updated_at = now()
    WHERE id = ${id}
  `;
...
/* A handle to offer someone on first sign-in. Derived from their name, or
   the local part of their email, and nudged with a number until it is free.
   Only a suggestion — setHandle still validates whatever they submit. */
export async function suggestHandle({ firstName, lastName, email }) {
  const base = ([firstName, lastName].filter(Boolean).join('-') || String(email || '').split('@')[0])
    .toLowerCase()
  ...
  for (let n = 0; n < 60; n++) {
    ...
    if (handleProblem(candidate)) continue;
    const taken = await sql`SELECT 1 FROM users WHERE lower(handle) = ${candidate} LIMIT 1`;
